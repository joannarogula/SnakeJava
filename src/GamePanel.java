...
public class GamePanel extends JPanel implements ActionListener {
    ...
    GamePanel() {
        ...
        // Start the AI Snake in a separate thread
    ...
    void makeAiMove() {
        Direction newDirection = aiDirection;
        int minDistance = Integer.MAX_VALUE;

        for (Direction dir : Direction.values()) {
            int newX = aiXCoord[0] + dir.getXOffset() * UNIT_SIZE;
            int newY = aiYCoord[0] + dir.getYOffset() * UNIT_SIZE;

            if (newX >= 0 && newX < WIDTH && newY >= 0 && newY < HEIGHT) {
                int distance = Math.abs(newX - xFruits[0]) + Math.abs(newY - yFruits[0]);
        ...
        aiXDirection = aiDirection.getXOffset() * UNIT_SIZE;
        aiYDirection = aiDirection.getYOffset() * UNIT_SIZE;
    }
    ...
    // private class AiSnakeRunnable implements Runnable {
    //     @Override
    //     public void run() {
    //         while (isRunning) {
    //             makeAiMove();
